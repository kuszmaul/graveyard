...
class EnumsAndStrings {
 public:
  ...
  static const EnumsAndStrings* Create(std::vector<std::pair<EnumType, absl::string_view>> pairs) {
    // No static variables with nontrivial destructors.
    static const EnumsAndStrings* enums_and_strings = new EnumsAndStrings(std::move(pairs));
  ...
  absl::string_view ToString(EnumType enum_type) const {
    ...
    CHECK(false) << "This shouldn't happen.  Perhaps there is a missing pair when calling the EnumsAndStrings constructor";
  }
  ...
  class JoinFormatter {
   public:
    JoinFormatter(const EnumsAndStrings &enums_and_strings) :enums_and_strings_(&enums_and_strings) {}
  ...
  const std::vector<std::pair<EnumType, absl::string_view>>& Pairs() const { return pairs_; }
 ...
 private:
  EnumsAndStrings(std::vector<std::pair<EnumType, absl::string_view>> pairs) :pairs_(std::move(pairs)) {}
