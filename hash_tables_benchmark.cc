...
ABSL_FLAG(std::vector<std::string>, tables, std::vector<std::string>({"OLP", "OLP-idhash", "google", "google-idhash", "facebook", "facebook-idhash"}),
...
int main(int argc, char *argv[]) {
  ...
  absl::flat_hash_set<std::string> tables;
  {
    ...
    tables = absl::flat_hash_set<std::string>(tables_vector.begin(), tables_vector.end());
  }
  ...
  if (tables.contains("facebook-idhash")) {
    ...
    IntHashSetBenchmark<F14NoHash>(F14MemoryEstimator<F14NoHash>,
                             "facebook");
  }
}
