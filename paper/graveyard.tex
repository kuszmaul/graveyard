...
\todo{Issues: 
(1) rebuild-to-rebuild hash table not given enough credit.
(2) we are just saying what implicit graveyard does, but we are saying that yes.
(3) sequential versus multithreaded issues.
}
...
\todo{two issues: (1) clustering. (2) linear probing fixes are on not rebuild-to-rebuild stable and are slow.}

% % MAB: I know the title isn't yet right, but I'd love a paragraph heading
% \propose{
% \paragraph{Clustering in linear probing.}
% }
...
% Interestingly, it has been known for decades how to modify linear probing in order to mitigate the effects of primary clustering \emph{for queries}. Indeed, in 1973, Amble and Knuth introduced so-called \defn{ordered linear probing}, which arranges the elements within each run in such a way that queries can reliably terminate while only having seen a small fraction of the run. This optimization helps queries (at least in theory) but not insertions.

% \propose{\paragraph{xxx}}

% \todo{In fact, there are different clustering effects for insertions and queries, and different approaches for avoiding them.... }
...
% The original \texttt{C++} standard library achieved it For example, Rebuild-to-rebuild stability ensures that iterators within the hash table remain valid between rebuilds. However, rebuild-to-rebuild stability is incompatible with many hash table designs, including cuckoo hashing \cite{}, robin hood hashing \cite{?}, and intuitively graveyard hashing as well. This is one of the main technical challenges that we overcome in this paper.
...
\todo{Could introduce vectorization here if we want...}
...
tombstones improve insertions and unsuccessful lookups.  \todo{State
  the load factors used by graveyard.}
